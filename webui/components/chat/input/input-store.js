...
const model = {
  ...
  async pauseAgent(paused) {
    ...
    try {
      ...
      if (globalThis.toastFetchError) {
        globalThis.toastFetchError('Error pausing agent', _e);
      }
    }
  },
  ...
  async nudge() {
    try {
      ...
      if (globalThis.toastFetchError) {
        globalThis.toastFetchError('Error nudging agent', _e);
      }
    }
  },
    ...
    input.onchange = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          if (globalThis.toast) {
            globalThis.toast('Knowledge files imported: ' + data.filenames.join(', '), 'success');
          }
        }
      } catch (_e) {
        if (globalThis.toastFetchError) {
          globalThis.toastFetchError('Error loading knowledge', _e);
        }
      }
    };
