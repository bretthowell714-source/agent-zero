...
const model = {
  ...
  async saveSelectedProject(creating) {
    try {
      ...
      for (const kvp of Object.entries(data)) if (kvp[0].startsWith('_')) delete data[kvp[0]];
  ...
  async browseInstructionFiles() {
    ...
    try {
      const newData = await this._createEditProjectData(this.selectedProject.name);
      this.selectedProject.instruction_files_count = newData.instruction_files_count;
    } catch (error) {
      //pass
    }
  },
  ...
  async browseKnowledgeFiles() {
    ...
    try {
      ...
      const newData = await this._createEditProjectData(this.selectedProject.name);
      this.selectedProject.knowledge_files_count = newData.knowledge_files_count;
