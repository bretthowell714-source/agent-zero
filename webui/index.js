...
export async function sendMessage() {
  ...
  try {
    ...
    if (message || hasAttachments) {
      ...
      if (hasAttachments) {
        const heading =
          attachmentsWithUrls.length > 0 ? 'Uploading attachments...' : 'User message';
...
export function updateAfterScroll() {
  ...
  const isAtBottom =
    chatHistory.scrollHeight - chatHistory.scrollTop <= chatHistory.clientHeight + tolerancePx;
    ...
    if (settingsButton) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const schedulerData = globalThis.Alpine ? Alpine.$data(schedulerComponent) : null;
